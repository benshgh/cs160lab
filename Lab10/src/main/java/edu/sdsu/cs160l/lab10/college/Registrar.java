...
import edu.sdsu.cs160l.lab10.exception.IllegalCourseNameException;
import edu.sdsu.cs160l.lab10.exception.StudentAlreadyEnrolledException;
import edu.sdsu.cs160l.lab10.student.*;
import edu.sdsu.cs160l.lab10.student.path.GEPathBehavior;
import edu.sdsu.cs160l.lab10.student.path.ILearningPathBehavior;
import edu.sdsu.cs160l.lab10.student.path.LowerDivisionPathBehavior;
...
public class Registrar implements IRegistrar {
	...
	public void enrollStudentCourse(IStudentStatus stu, String course)
			throws ClassFullException, StudentAlreadyEnrolledException, IOException {
		...
		if (stuLst.size() == CSCourseFactory.MAX_CLASS_SIZE) {
			throw new ClassFullException(
					course + " Class size is full, " + "failed adding " + stu.getStudentID() + " to the class.");
		}

		// Throw a checked StudentAlreadyEnrolledException if the student
		// is already enrolled in the class

		// use the Stream way (the same as above CSCourseFactory.IsCourseValid)
	...
	/*
	 * use stream to conduct functional programming exercises for map, reduce,
	 * filter
	 */
	 ...
	 * @return number of students enrolled in a course where the students are of the
	 *         major passed-in
	 */
	public long getNumOfStudentsEnrolled(StudentMajor major, String course) {
		...
		return stuLst.stream().filter(s -> s.getStudentMajor() == major).count();
	}
	 ...
	 * @param course course name
	 * @return number of students enrolled in a course where the students are of the
	 *         student level passed-in
	 */
	public int getNumOfStudentsEnrolled(StudentLevel sl, String course) {

		// use the similar stream operations as in the above getNumOfStudentsEnrolled
	...
	/**
	 * Returns number of students enrolled in a course where the students are of a
	 * particular major and a particular student level
	 * <p>
	 * <p>
	 * Assumption:
	 *
	 ...
	 * @param course course name
	 * @return number of students enrolled in a course where the students are of the
	 *         student major and the student level passed-in
	 */
	...
	/**
	 * Returns a list of IDs of students enrolled in a group of courses of a certain
	 * level where the students are of a particular major
	 * <p>
	 * <p>
	 * Assumption:
	 *
	 * @param major          student major
	 * @param courseStarting courses that start with, e.g., "CS2" meaning all
	 *                       courses starting with "CS2"
	 * @return List of IDs of students enrolled in the courses starting with
	 *         courseStarting, e.g., CS5XX (CS500 level courses: CS530, CS514,
	 *         etc.). Note in the case of a student enrolled into multiple CS5XX
	 *         course, its student ID should be added to the return list ONCE.
	 */
	public List<Integer> getStudentsEnrolled(StudentMajor major, String courseStarting) {

		/*
		 * Note if students have trouble in understanding ONE sequence of streamlined
		 * operations, they could also use the split implementation followed (which I
		 * commented out here).
		 */
		return this.courseStudents.entrySet() // Map.Entry<String, List<IStudentStatus>>
		 ...
		 * .filter(c -> c.getKey().startsWith(courseStarting)) .map(Map.Entry::getValue)
		 * .collect(Collectors.toList());
		 * 
		 * // flatmap flattens all List<IStudentStatus> in lsts to // individual
		 * IStudentStatus objects.
		 * 
		 * // flattening means following transformation: // using an integer array for
		 * illustration // before flattening - [ [1, 3], [5, 17], [22, 65] ] // after
		 * flattening - [ 1, 3, 5, 17, 22, 65 ] List<IStudentStatus> concatLst =
		 * lsts.stream() .flatMap(l -> l.stream()) .collect(Collectors.toList());
		 * 
		 * //Note distinct() is used here for only counting a student once return
		 * concatLst.stream() .filter(s -> s.getStudentMajor() == major) .map(ss ->
		 * ss.getStudentID()) .distinct() .collect(Collectors.toList());
		 */
	}

	/**
	 * TODO: this method is used for Lab 10 sorting programming exercises using
	 * lambdas in Lab10Test.java Returns a list of IStudentStatus of students
	 * enrolled in a group of courses
	 * <p>
	 * <p>
	 * Assumption:
	 *
	 ...
	 *         courseStarting, e.g., CS5XX (CS500 level courses: CS530, CS514,
	 *         etc.). Note in the case of a student enrolled into multiple CS5XX
	 *         course, its IStudentStatus object should be added to the return list
	 *         ONCE.
	 */
	public List<IStudentStatus> getStudentsEnrolled(String courseStarting) {
	...
	/**
	 * Returns the grades of students of a course where the students are of a
	 * particular major
	 * <p>
	 * <p>
	 * Assumption:
	 ...
	 * @param major  student major
	 * @param course course name
	 * @return grades of the students enrolled in a course where the students are of
	 *         a particular major
	 */
	...
	/**
	 * Returns the average grade of students of a course where the students are of a
	 * particular student level
	 * <p>
	 * <p>
	 * Assumption:
	 ...
	 * @param course course name
	 * @return average grade of the students in a course where the students are of a
	 *         particular student level
	 */
	public double getAvgGrade(StudentLevel level, String course) {
		...
		OptionalDouble avgGrade = stuGrades.entrySet().stream()
				...
				.mapToDouble(s -> s.getValue().getGrade()).average();
	...
	/**
	 * Returns the top N students (in gpa) of a course where the students are of a
	 * particular major
	 * <p>
	 * <p>
	 * Assumption:
	...
	/**
	 * TODO: this method is used for Lab 10 sorting programming exercises using
	 * lambdas in Lab10Test.java Returns the list of StudentGrade of a course
	 * <p>
	 * <p>
	 * Assumption:
	 *
	 * @param course course name
	 * @return grades of the students enrolled in a course
	 */
	public List<StudentGrade> getStudentGrades(String course) {

		// get the map of Student Grades given the course.
		Map<Integer, StudentGrade> stuGrades = this.courseGrades.get(course);

		if (stuGrades == null)
			return null; // later consider to return an empty list

		// use filter and map to get the list of Students(IStudentStatus) of
		// the major passed-in
		List<StudentGrade> sGrades = stuGrades.entrySet().stream().map(Map.Entry::getValue)
				.collect(Collectors.toList());
	...
	/**
	 * Returns the average grade of students enrolled in a group of courses of a
	 * certain course level (CS4XX, CS5XX, or ...) where the students are of a
	 * particular major
	 * <p>
	 * <p>
	 * Assumption:
	 *
	 * @param major          student major
	 * @param courseStarting course Starting, e.g., "CS2" meaning all courses
	 *                       starting with "CS2"
	...
	public double getAvgGradeCourseLevel(StudentMajor major, String courseStarting) {
		...
		// get the list of the Hashmaps of the students' grades in the courses of the
		// course level
	...
	/**
	 * Returns List of StudentUnits objects of a learning path for students of a
	 * particular major and a particular level
	 * <p>
	 * <p>
	 * Assumption:
	 *
	 ...
	 * @return A list of StudentUnits for students of a particular major and/or a
	 *         particular level
	 */
	public List<StudentUnits> getStudentUnits(StudentMajor major, StudentLevel level, LearningPathType lPathType) {
	...
	/**
	 * TODO: this method is used for Lab 10 sorting programming exercises using
	 * lambdas in Lab10Test.java Returns List of StudentUnits objects of a learning
	 * path
	 * <p>
	 * <p>
	 * Assumption:
	 *
	 * @param lPathType student learning path
	 * @return A list of StudentUnits completed for the specified learning path
	 */
	public List<StudentUnits> getStudentUnits(LearningPathType lPathType) {

		// use filter to sort out the StudentUnits for the passed-in learning path
		return this.studentUnitsTaken.entrySet().stream().filter(su -> su.getValue().containsKey(lPathType))
	...
	public int getStudentTotalUnits(int stuID) {
		...
		return su.entrySet().stream().map(Map.Entry::getValue).map(s -> s.getUnits()).reduce(0,
				(subtotal, nUnits) -> subtotal + nUnits);
	}

	/**
	 * Returns Average number of course units students of a major have taken for a
	 ...
	 * @param major     student major
	 * @param lPathType student learning path
	 * @return Average number of course units students of a major have taken for the
	 *         given learning path
	 */
	public double getAvgCourseUnits(StudentMajor major, LearningPathType lPathType) {
		...
		OptionalDouble avgUnits = this.studentUnitsTaken.entrySet().stream()
				.filter(su -> su.getValue().containsKey(lPathType)).map(Map.Entry::getValue) // get
																								// Map<LearningPathType,
																								// StudentUnits>
				.flatMap(su -> su.values().stream()) // flattern each map --> List<StudentUnits>
				.filter(su -> (su.getStudentStatus().getStudentMajor() == major)).mapToDouble(su -> su.getUnits())
				.average();
	...
	/**
	 * Returns Max number of course units students of a level have taken for a
	 * particular learning path
	 * <p>
	 * <p>
	 * Assumption:
	 ...
	 * @param level     student level
	 * @param lPathType student learning path
	 * @return Max number of course units students of a level have taken for the
	 *         given learning path
	 */
	public int getMaxCourseUnits(StudentLevel level, LearningPathType lPathType) {
	...
	/**
	 * Returns Average number of course units students of non-computing majors have
	 * taken for a learning path
	 * <p>
	 * <p>
	 * Assumption:
	 *
	 * @param lPathType student learning path
	 * @return Average number of course units students of non-computing majors have
	 *         taken for a learning path
	 */
	...
	/**
	 * Implement the function to notify all subscribed students about a learning
	 * path change
	 */
